...
import { X, Music, Upload, Loader2, CheckCircle2, XCircle } from 'lucide-react';
...
export function CsvImportModal({
  ...
  const handleFile = async (file: File) => {
    ...
    const pickBestMatch = (linesList: string[], title: string, artist: string): string | null => {
      ...
      if (!bestId) {
        const id = linesList[0]?.split('====')[3]?.trim();
        bestId = id || null;
      }
      return bestId;
    };
    ...
    const processTrack = async (track: typeof initial[0], i: number): Promise<void> => {
      ...
      try {
        ...
        if (cleanId === undefined) {
          ...
          const linesList = res.trim().split('\n').filter(Boolean).slice(0, 5);
          cleanId = pickBestMatch(linesList, track.title, track.artist);
