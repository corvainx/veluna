  ...
  return (
    <div style={{ width: "100%", position: "relative" }}>
      ...
      >
        ...
        <span style={{ fontSize: "12px", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap", flex: 1, textAlign: "left", color: "rgba(255,255,255,0.7)" }}>
        ...
        <div style={{ position: "absolute", bottom: "calc(100% + 6px)", left: 0, right: 0, borderRadius: "10px", overflow: "hidden", zIndex: 9999, background: "var(--v-bg2)", border: "1px solid rgba(255,255,255,0.1)", boxShadow: "0 16px 40px rgba(0,0,0,0.9)" }}>
          ...
          {devices.map(dev => (
            <button
              ...
              disabled={switching}
              onClick={() => {
                if (!dev.is_default) onSwitch(dev.id);
                setOpen(false);
              }}
              style={{
                width: "100%",
                ...
                textAlign: "left",
                border: "none",
                background: dev.is_default ? "rgba(255,255,255,0.05)" : "transparent",
                cursor: dev.is_default ? "default" : "pointer",
                transition: "background .1s"
              }}
              onMouseEnter={e => {
                if (!dev.is_default) (e.currentTarget as HTMLElement).style.background = "rgba(255,255,255,0.05)";
              }}
              onMouseLeave={e => {
                if (!dev.is_default) (e.currentTarget as HTMLElement).style.background = "transparent";
              }}
            >
              <div style={{ width: "6px", height: "6px", borderRadius: "50%", flexShrink: 0, background: dev.is_default ? "#9e9894" : "rgba(255,255,255,0.15)" }} />
              <span style={{ fontSize: "12px", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap", color: dev.is_default ? "#fff" : "rgba(255,255,255,0.5)", flex: 1, textAlign: "left" }}>
                {dev.name}
              </span>
